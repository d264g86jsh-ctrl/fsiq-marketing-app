...
type ActionFamily = 'recommendation' | 'script' | 'topic' | 'variation' | 'test'
...
export async function POST(req: NextRequest) {
  ...
  if (payload.type === 'view_submission') {
    ...
    const pipelineId = vp.view.private_metadata
    const editedScript = vp.view.state.values.script_edit_block?.script_text?.value ?? ''
    const actorName = vp.user?.name ?? vp.user?.id ?? 'unknown'

    if (pipelineId && editedScript) {
      const { data: pipeline } = await supabase
        .from('creative_pipeline')
        .update({ script_draft: editedScript })
        .eq('id', pipelineId)
        .select('concept_name, slack_ts, slack_channel')
        .single()

      // Post revised script as thread reply if we have the original message coords
      if (pipeline?.slack_ts && pipeline?.slack_channel) {
        const preview = editedScript.length > 600
          ? editedScript.slice(0, 597) + '…'
          : editedScript
    ...
    // Returning response_action: clear closes the modal
